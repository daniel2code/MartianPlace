...
export default function Create() {
  ...
  }, [image, user, buffer, nftName, description, showProperties, properties, royalties, project, priceInitial, priceNow, address, url])
      ...
      <div className='mx-5 md:mx-40 sm:mx-10 mt-20 mb-5'>
        ...
        <div className={`${styles.element} md:flex md:flex-row mt-10 md:mt-6`}>
          <FileUploaderDND changeInputFile={setImageAction} otherstyles="justify-self-start"
            ...
            img2={image.image} />
          {image.image ?
            <div className={`${styles.nftImage} mt-10 md:mt-5 text-center sm:mt-10 md:text-left`}>
              <Image src={image.image.file} alt={'nft image'} width={280} height={280}></Image> </div>
            : null}
        </div>
        ...
        <div className={`${styles.element} mt-12 md:mt-8 border-b border-b-1 border-b-dark-1 flex flex-row`}>
        ...
        <div className={`${styles.element} mt-6 md:mt-3`}>
          ...
          <div className={`mt-8 md:mt-0 flex flex-row`}>
            <span >
              {'What unique features make up your NFT? (Add to list below)'} </span>
            {(properties.length > 0) && <a title='Collapse Properties' className='ml-4 md:ml-2 text-purple-primary underline'
              onClick={() => setShowProperties(!showProperties)}> Collapse</a>}
          </div>
          ...
          <div >
            <Button
              ...
              onClick={() => { handleAdd(properties, setProperties); setShowProperties(true) }}
              label={`Add a Property`} />
          </div>
          <div className={`${showProperties ? 'visible md:grid md:grid-cols-2 md:gap-6 mt-8 md:mt-4' : 'hidden'}`}>
            {properties.map((field, idx) => {
              return (
                <div key={`${field}-${idx}`} className={`${styles.outline} rounded-md border-1 mb-10 md:mb-0 ${styles.element}`}
                >
                  <div className={`md:flex md:flex-row`}>
                    <div className={`md:flex-1 md:mr-1 mt-10 md:mt-6`}>
                      <Input inputstyles={`shadow-inner border-none focus:outline-none ${styles.inputWidth} ${styles.inputWrapper}`}
                        placeHolder={`Type`} title="type of property" parentstyles={`rounded-md border border-1 border-dark-1 bg-nft-input-bg px-3 py-1.5`}
                        value={properties[idx]['Type']} onChange={(val) => {
                          handleChangeInput(idx, val, 'Type', properties, setProperties);
                        }} />
                    </div>
                    <div className={`md:flex-1 md:ml-1 mt-10 md:mt-6`}>
                      <Input inputstyles={`shadow-inner border-none focus:outline-none ${styles.inputWidth} ${styles.inputWrapper}`}
                        placeHolder={`Value`} title="value of property" parentstyles={`rounded-md border border-1 border-dark-1 bg-nft-input-bg px-3 py-1.5`}
                        value={properties[idx]['Value']} onChange={(val) => {
                          handleChangeInput(idx, val, 'Value', properties, setProperties);
                        }} />
                    </div>
                  </div>
                  <div >
                    <Button
                      ...
                      onClick={() => { handleRemove(idx, properties, setProperties) }}
                      label={`Remove`} />
                  </div>
                </div>
              );
        ...
        <div className={`${styles.element} mt-10 md:mt-6 flex flex-row mb-2 pt-1`}>
          <Button
            type='button'
            id={`create`}
            title={`create`}
            ...
            onClick={() => { createNFT() }}
          >{(isLoading && !url) ? <div className='h-[10px] pb-12 ml-4'><Ring size={32} color={'#fafafa'} sizeUnit={'px'} /> </div> : 'Create'}
          </Button>
